...
export class RequisitionService {
    ...
    try {
      ...
      if (orders.some((order) => !!order.requisitionId)) {
        throw new ApiError(
          ...
          'Some orders are already in a requisition',
        );
      }
      ...
      const savedRequisition = await queryRunner.manager.save(
        Requisition,
        requisition,
      );
      ...
      for (const product of allProducts) {
        ...
        const locationId = product.orderId
          ? orders.find((o) => o.id === product.orderId)?.locationId
          : null;
        ...
        if (locationId) {
          totals.byLocation.set(
            locationId,
            (totals.byLocation.get(locationId) || 0) + qty,
          );
        }
      }
  ...
  async generateRequisitionNumber(): Promise<string> {
    ...
    if (latestRequisition) {
      const lastNumber = parseInt(
        latestRequisition.requisitionNumber.split('-').pop() || '0',
        10,
      );
  ...
  async generatePreview(orderIds: number[], locationId: string) {
    if (!orderIds.length) return { products: [] };

    // 1️⃣ Fetch all orders + products + product details in one query
    const rows = await this.orderRepository
      .createQueryBuilder('order')
      .leftJoinAndSelect('order.products', 'op')
      .leftJoinAndSelect('op.product', 'product')
      .where('order.id IN (:...orderIds)', { orderIds })
      .getMany();

    // 2️⃣ Flatten products
    ...
    // 3️⃣ Fetch all inventories for these products + location
    ...
    const inventoryMap = new Map(
      inventoryItems.map((item) => [item.productId, item.quantity || 0]),
    );

    // 4️⃣ Merge stock info
    products.forEach((p) => {
      p.stock = inventoryMap.get(p.productId) || 0;
    });
