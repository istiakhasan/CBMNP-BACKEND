...
export class DeliveryPartnerService {
  ...
  async create(createDeliveryPartnerDto: any) {

    const isPartnerAlreadyExist=await this.deliveryPartnerRepository.findOne({where:{
      organizationId:createDeliveryPartnerDto.organizationId,
      partnerName:createDeliveryPartnerDto?.partnerName
    }})

    if(isPartnerAlreadyExist){
      throw new ApiError(HttpStatus.BAD_REQUEST,'Partner already exist')
    }
  ...
  async getPartner(options, filterOptions, organizationId) {
    ...
    const queryBuilder = this.deliveryPartnerRepository
    ...
    if (filterOptions?.searchTerm) {
      ...
      queryBuilder.andWhere(
        '(LOWER(partner.contactPerson) LIKE :searchTerm OR LOWER(partner.partnerName) LIKE :searchTerm)',
        { searchTerm },
      );
    }
