...
export class DeliveryPartnerController {
  ...
  @Post('create')
  create(
    @Body() createDeliveryPartnerDto: CreateDeliveryPartnerDto,
    @Req() req: Request,
  ) {
  ...
  async getPartner(@Query() query, @Req() req: Request) {
    ...
    return catchAsync(async (): Promise<IResponse<DeliveryPartner[]>> => {
      ...
      const filterOptions = extractOptions(query, [
        'searchTerm',
        'filterByCustomerType',
      ]);
      const result = await this.deliveryPartnerService.getPartner(
        paginationOptions,
        filterOptions,
        organizationId,
      );
  ...
  async warehouseOptions(@Req() req: Request) {
    ...
    return catchAsync(
      async (): Promise<IResponse<{ label: string; value: string }[]>> => {
        const result = await this.deliveryPartnerService.loadOptions(
