...
export class CustomerController {
  ...
  async createEmployee(
    @Body(new ZodPipe(CreateCustomerSchema)) data,
    @Req() req: Request,
    @Query() query,
  ) {
  ...
  async getCustomerRetentionReports(@Query() query, @Req() req: Request) {
    ...
    const organizationId = req.headers['x-organization-id'];
    const result = await this.customerService.getCustomerRetentionReports(
      options,
      searchFilterOptions,
      organizationId,
    );
...
async updateAddressBook(
  ...
  async topCustomersReprots(@Query() query, @Req() req: Request) {
    ...
    const result = await this.customerService.topCustomersReports(
      organizationId,
      options,
      searchFilterOptions,
    );
